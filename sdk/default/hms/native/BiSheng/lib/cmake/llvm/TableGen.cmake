...
set(TARGET_GLOBAL_TDS "" CACHE INTERNAL "TARGET_GLOBAL_TDS" FORCE)
...
function(tablegen project ofn)
  ...
  if (IS_ABSOLUTE ${ofn})
    ...
    set(OFN "${CMAKE_CURRENT_BINARY_DIR}/${ofn}")
  endif()
  ...
  if (LLVM_TARGETS_TO_BUILD MATCHES "HiIPU")
    list(APPEND tblgen_change_flag "-DBSPRIV_DAVINCI")
  endif()
  if (LLVM_ENABLE_CLASSIC_FLANG)
    list(APPEND tblgen_change_flag "-DENABLE_CLASSIC_FLANG")
  endif()

  if(LLVM_ENABLE_OPENMP_FFRT)
    list(APPEND tblgen_change_flag "-DBSPRIV_COMMON_OPENMP_FFRT")
  endif()
  ...
  if (LLVM_BSPUB_RISCV)
    list(APPEND tblgen_change_flag "-DBSPUB_RISCV")
  endif()
  ...
  # WARNING: Must include CMAKE_BINARY_DIR/include and
  # CMAKE_BINARY_DIR/lib/Target/HiIPU due to
  # 'include "llvm/IR/cce_aicore_intrinsics_ir.include"' in IntrinsicsHIVM.td
  # and 'include "cce_aicore_intrinsics_info.include"' in HiIPU*.td files.
  ...
  if (BSPUB_RISCV AND LLVM_TARGETS_TO_BUILD MATCHES "RISCV")
    add_custom_command(OUTPUT ${CMAKE_CURRENT_BINARY_DIR}/${ofn}
      COMMAND ${tablegen_exe} ${ARG_UNPARSED_ARGUMENTS}  -I ${LLVM_AUTOGEN_DIR} -I ${CMAKE_CURRENT_SOURCE_DIR}
      ${tblgen_includes}
      ${LLVM_TABLEGEN_FLAGS}
	...
	-I ${CMAKE_BINARY_DIR}/lib/Target/HiIPU
    ${LLVM_TABLEGEN_FLAGS}
    ${LLVM_TARGET_DEFINITIONS_ABSOLUTE}
    ${tblgen_change_flag}
    ${additional_cmdline}
  ...
  set_source_files_properties(${OFN} PROPERTIES
    GENERATED 1)
endfunction()
