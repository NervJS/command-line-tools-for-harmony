...
#if defined(BSPUB_COMMON)
      EscapingInstructions.clear();
      EscapeRoots.clear();
      TerminalInstrs.clear();
      VisitedUses.clear();
...
#if defined(BSPUB_COMMON)
    SetVector<Instruction *> &getEscapingInstructions() {
      return EscapingInstructions;
    }
#endif
...
#if defined(BSPUB_COMMON)
    void setVisitedUses(const SetVector<Use *> &VUs) { VisitedUses = VUs; }
    SetVector<Use *> &getVisitedUses() { return VisitedUses; }

    void setEscapeRoots(const SetVector<Instruction *> &E) { EscapeRoots = E; }
    SetVector<Instruction *> &getEscapeRoots() { return EscapeRoots; }

    void setTerminalInstrs(const SetVector<Instruction *> &T) {
      TerminalInstrs = T;
    }
    SetVector<Instruction *> &getTerminalInstrs() { return TerminalInstrs; }

    void setTypeOf(Value *V, Type *Ty) { TypeOf[V] = Ty; }
    MapVector<Value *, Type *> &getTypeOfMap() { return TypeOf; }

    void setVisitedCallSites(const SetVector<CallBase *> &Calls) {
      VisitedCallSites = Calls;
    }
    SetVector<CallBase *> &getVisitedCallSites() { return VisitedCallSites; }
#endif

  private:
    PointerIntPair<Instruction *, 1, bool> AbortedInfo, EscapedInfo;
#if defined(BSPUB_COMMON)
    SetVector<Instruction *> EscapingInstructions;
    SetVector<Instruction *> EscapeRoots;
    SetVector<Instruction *> TerminalInstrs;
    SetVector<Use *> VisitedUses;
    SetVector<CallBase *> VisitedCallSites;
    MapVector<Value *, Type *> TypeOf;
#endif
  };
...
#if defined(BSPUB_COMMON)
  /// Add the use \p U to the work list.
  void enqueueUse(Use &U);
#endif
