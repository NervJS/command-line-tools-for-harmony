...
struct DOTGraphTraits<DOTFuncInfo *> : public DefaultDOTGraphTraits {
  ...
  static int calls(BasicBlock *BB) {
    int C = 0;
  ...
  static int missEvents(BasicBlock *BB) {
    unsigned int CacheMisses = 0;
    for (Instruction &I : *BB) {
      if (MDNode *MD = I.getMetadata("l2icachemisses")) {
        Metadata *Data = MD->getOperand(0).get();
        if (ConstantAsMetadata *CAM = dyn_cast<ConstantAsMetadata>(Data)) {
          if (ConstantInt *CI = dyn_cast<ConstantInt>(CAM->getValue())) {
            assert(!CI->isNegative() && "unexpected negative cache miss count");
            CacheMisses += CI->getZExtValue();
          }
        }
      }
    }
  ...
  static std::string getSimpleNodeLabel(const BasicBlock *Node, DOTFuncInfo *) {
    ...
    if (!Node->getName().empty()) {
      OS << Node->getName() << " Size: " << Sz;
      OS << " Calls:" << calls(const_cast<BasicBlock *>(Node));
      OS << " MissEvents:" << missEvents(const_cast<BasicBlock *>(Node));
      return OS.str();
    }
#else
    if (!Node->getName().empty())
    ...
    OS << " Size:" << Sz;
    OS << " Calls:" << calls(const_cast<BasicBlock *>(Node));
    OS << " Misses:" << missEvents(const_cast<BasicBlock *>(Node));
#endif
    return OS.str();
  }
