...
#include <functional>
#include <utility>
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON_VECTOR_COMBINE)
...
public:
...
#if defined(BSPRIV_COMMON_LOOP_VECTORIZE)
  /// Return true if the target traps divide-by-zero code.
  bool isDivideByZeroUnsafe(unsigned Opc) const;

  bool enableUnrollThrottling() const;
#endif

#if defined(BSPUB_COMMON)
#if defined(BSPRIV_COMMON_VECTOR_COMBINE)
  /// \returns True if the specified extend load for the given type is legal.
  bool isExtLoadLegal(unsigned Opcode, Type *Dst, Type *Src) const;

  Optional<std::tuple<Intrinsic::ID, Value *, Value *>>
  useAbsDiffIntrinsic(Instruction &I) const;

  Optional<std::tuple<Intrinsic::ID, Value *>>
  useDotProductInReduction(const IntrinsicInst &II) const;
#endif
#endif
  ...
  bool isCheckCtlzOperatingOnZero() const;
  /// @}

  /// @}
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON_LOOP_DATA_PREFETCH)
  /// If you need to get some target-specific intrinsic whether suported or
  /// not When you want to do some optimization, you can use this interface to
  /// do it, but you need to modify the code based on your requirement. Now just
  /// support the check of Intrinsic::masked_prefetch, other return zeros.
  unsigned getTargetSupportedIntrinsic(const Intrinsic::ID IID) const;
#endif
...
#if defined(BSPRIV_COMMON_LOOP_VECTORIZE)
  virtual bool enableUnrollThrottling() const = 0;
  virtual bool isDivideByZeroUnsafe(unsigned Opc) const = 0;
#endif
#if defined(BSPUB_COMMON)
#if defined(BSPRIV_COMMON_VECTOR_COMBINE)
  virtual bool isExtLoadLegal(unsigned Opcode, Type *Dst, Type *Src) const = 0;
  virtual Optional<std::tuple<Intrinsic::ID, Value *, Value *>>
  useAbsDiffIntrinsic(Instruction &I) const = 0;
  virtual Optional<std::tuple<Intrinsic::ID, Value *>>
  useDotProductInReduction(const IntrinsicInst &II) const = 0;
#endif
#if defined(BSPRIV_COMMON_LOOP_DATA_PREFETCH)
  virtual unsigned getTargetSupportedIntrinsic(const Intrinsic::ID IID) const = 0;
#endif
#endif

  virtual bool isCheckCtlzOperatingOnZero() const = 0;
};
...
#if defined(BSPRIV_COMMON_LOOP_VECTORIZE)
  bool enableUnrollThrottling() const override {
    return Impl.enableUnrollThrottling();
  }

  bool isDivideByZeroUnsafe(unsigned Opc) const override {
    return Impl.isDivideByZeroUnsafe(Opc);
  }
#endif

#if defined(BSPUB_COMMON)
#if defined(BSPRIV_COMMON_VECTOR_COMBINE)
  bool isExtLoadLegal(unsigned Opcode, Type *Dst, Type *Src) const override {
    return Impl.isExtLoadLegal(Opcode, Dst, Src);
  }

  Optional<std::tuple<Intrinsic::ID, Value *, Value *>>
  useAbsDiffIntrinsic(Instruction &I) const override {
    return Impl.useAbsDiffIntrinsic(I);
  }

  Optional<std::tuple<Intrinsic::ID, Value *>>
  useDotProductInReduction(const IntrinsicInst &II) const override {
    return Impl.useDotProductInReduction(II);
  }
#endif

#if defined(BSPRIV_COMMON_LOOP_DATA_PREFETCH)
  unsigned getTargetSupportedIntrinsic(const Intrinsic::ID IID) const override {
    return Impl.getTargetSupportedIntrinsic(IID);
  }
#endif

#endif

  bool isCheckCtlzOperatingOnZero() const override {
    return Impl.isCheckCtlzOperatingOnZero();
  }
};
