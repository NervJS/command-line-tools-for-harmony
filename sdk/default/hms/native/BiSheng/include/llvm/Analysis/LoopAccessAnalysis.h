...
public:
  ...
  void insert(Loop *Lp, Value *Ptr, const SCEV *PtrExpr, Type *AccessTy,
              ...
              bool AllowUncountedLoops = false);
#else
              PredicatedScalarEvolution &PSE, bool NeedsFreeze);
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON)
  /// Returns the value of AllowUncountedLoops flag.
  bool isAllowUncountedLoops() const { return AllowUncountedLoops; }
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON)
  const LoopAccessInfo &getInfo(Loop *L, bool AllowUncountedLoops = false);
#else
