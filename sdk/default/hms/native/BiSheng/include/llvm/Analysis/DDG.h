...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON_DDG)
...
// wrapper class for the legacy pass manager
class DDGWrapperPass : public LoopPass {

public:
  static char ID;

  DDGWrapperPass() : LoopPass(ID) {
    initializeDDGWrapperPassPass(*PassRegistry::getPassRegistry());
  }

  DataDependenceGraph &getDataDependenceGraph() { return *DDG; }
  const DataDependenceGraph &getDataDependenceGraph() const { return *DDG; }
