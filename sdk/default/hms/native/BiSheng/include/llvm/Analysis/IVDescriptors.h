...
public:
  ...
  RecurrenceDescriptor(Value *Start, SmallVectorImpl<Instruction *> &Exits,
                       StoreInst *Store, RecurKind K, FastMathFlags FMF,
                       Instruction *ExactFP, Type *RT, bool Signed,
                       bool Ordered, SmallPtrSetImpl<Instruction *> &CI,
                       unsigned MinWidthCastToRecurTy)
      : IntermediateStore(Store), StartValue(Start),
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON)
                  bool AllowMultipleExits, // BISHENG: reduce delta
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON)
  // For the 'regular' loop vectorizer
  ...
  const ExitInstrList *getLoopExitInstrs() { return &LoopExitInstrs; }
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON)
  // List of instructions who's values are used outside the loop.
