...
#include <deque>
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON_REASSOCIATE)
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON_REASSOCIATE)
  // This will hold the ranks for every instruction and function arguments
  // Instructions within same Loop or BasicBlock will get same rank.
  DenseMap<Value *, unsigned> LoopMap;
#endif
...
public:
  ...
  PreservedAnalyses runImpl(Function &F, LoopInfo *LI, ScalarEvolution *SE);
#else
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON_REASSOCIATE)  
  void findSingleUseMultiplyFactors(Value *V,
                                    SmallVectorImpl<Value *> &Factors);
  BinaryOperator *breakUpSubtract(Instruction *Sub,
                                  ReassociatePass::OrderedSet &ToRedo);
  Value *negateValue(Value *V, Instruction *BI,
                     ReassociatePass::OrderedSet &ToRedo);
  void buildLoopMap(Function &F, ReversePostOrderTraversal<Function *> &RPOT,
                    LoopInfo *LI, ScalarEvolution *SE);
  void processLoop(const Loop *L, LoopInfo *LI, ScalarEvolution *SE,
                   unsigned &Rank);
#endif
};
