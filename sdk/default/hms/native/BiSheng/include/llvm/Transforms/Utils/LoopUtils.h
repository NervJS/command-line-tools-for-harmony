...
Value *createOrderedReduction(IRBuilderBase &B,
                              ...
                              Value *Start, bool UsePredication = false,
                              Value *Mask = nullptr);
#else
                              Value *Start);
#endif
...
#if defined(BSPRIV_COMMON)
/// Return true if the graph for a given loop has a subloop and is acyclic.
/// Return through arguments subloop, forward, and aft basic blocks, as well
/// as the number of critical edges in the loop.
bool graphLoopBlocks(const Loop *L, const Loop *SubLoop,
                     SmallPtrSet<BasicBlock *, 4> &ForeBlocks,
                     SmallPtrSet<BasicBlock *, 4> &SubLoopBlocks,
                     SmallPtrSet<BasicBlock *, 4> &AftBlocks,
                     int &CriticalEdgeCount, bool PrintGraph = false);
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON_LOOP_VECTORIZE)
                 SCEVExpander &Expander,
                 const SmallVectorImpl<RuntimePointerChecking::PointerInfo>& Pointers,
                 unsigned PtrSafetyDistance = 0);
#else
                 SCEVExpander &Expander);
#endif
...
void collectInnerMostLoops(Loop *L, SmallVector<Loop *, 4> &V);

/// Find pointers that can possibly reduce most computations in a loop if they
/// have zero value
void findPointersThatCanReduceComputations(const Loop *L,
                                           SmallPtrSet<Value *, 4> &P);
#endif
