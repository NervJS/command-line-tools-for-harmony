...
template <typename T0, typename T1> struct SeriesVector_match {
  ...
  template <typename OpTy> bool match(OpTy *V) {
    ...
    // series_vector(0, 1)
    if (m_Intrinsic<Intrinsic::experimental_stepvector>().match(V))
      return Start.match(Zero) && Step.match(One);

    auto StartMod = m_SplatVector(m_Value(X));
    auto StepMod = m_SplatVector(m_Value(Y));
    auto ScaledStepModA = m_Mul(m_Intrinsic<Intrinsic::experimental_stepvector>(), StepMod);
    auto ScaledStepModB = m_Mul(StepMod, m_Intrinsic<Intrinsic::experimental_stepvector>());

    // series_vector(X, 1)
    if (m_Add(m_Intrinsic<Intrinsic::experimental_stepvector>(), StartMod).match(V) ||
        m_Add(StartMod, m_Intrinsic<Intrinsic::experimental_stepvector>()).match(V))
      return Start.match(X) && Step.match(One);

    // series_vector(0, Y)
    if (m_Mul(m_Intrinsic<Intrinsic::experimental_stepvector>(), StepMod).match(V) ||
        m_Mul(StepMod, m_Intrinsic<Intrinsic::experimental_stepvector>()).match(V))
