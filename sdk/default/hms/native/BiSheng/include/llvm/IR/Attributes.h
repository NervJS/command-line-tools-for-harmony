...
public:
  ...
  static Attribute getWithSparseField(LLVMContext &Context,
                                      uint64_t FieldIndex);
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON_INLINE)
  /// Returns the field index of struct from the sparse_field attribute.
  uint64_t getSparseFieldIndex() const;
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON_INLINE)
  /// Add the sparse_field attribute to the attribute set at the given index.
  /// Returns a new list because attribute lists are immutable.
  LLVM_NODISCARD AttributeList addSparseFieldAttr(LLVMContext &C,
                                                  unsigned Index,
                                                  uint64_t FieldIndex) const;

  /// Add the sparse_field attribute to the attribute set at the given arg
  /// index. Returns a new list because attribute lists are immutable.
  LLVM_NODISCARD AttributeList addSparseFieldParamAttr(
      LLVMContext &C, unsigned ArgNo, uint64_t FieldIndex) const {
    return addSparseFieldAttr(C, ArgNo + FirstArgIndex, FieldIndex);
  }
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON_INLINE)
  /// Get the field index of struct (or zero if unknown).
  uint64_t getSparseFieldIndex(unsigned Index) const;

  /// Get the field index of struct (or zero if unknown) of an arg.
  uint64_t getParamSparseFieldIndex(unsigned ArgNo) const {
    return getSparseFieldIndex(ArgNo + FirstArgIndex);
  }
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON_INLINE)
  /// Retrieve the field index of struct, if the
  /// sparse_field attribute exists (zero is returned otherwise).
  uint64_t getSparseFieldIndex() const {
    return getRawIntAttr(Attribute::SparseField);
  }
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON_INLINE)
  /// This turns the field index of struct into the form used internally in
  /// Attribute.
  AttrBuilder &addSparseFieldAttr(uint64_t FieldIndex);
#endif
