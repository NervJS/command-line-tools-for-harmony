...
static inline unsigned getFSPassBitBegin(sampleprof::FSDiscriminatorPass P) {
  ...
  unsigned I = static_cast<unsigned>(P);
  assert(I <= getNumFSPasses() && "Invalid FS discriminator pass number.");
  return getFSPassBitEnd(static_cast<sampleprof::FSDiscriminatorPass>(I - 1)) + 1;
}
#else
...
#if defined(BSPUB_COMMON)
  return getFSPassBitBegin(static_cast<sampleprof::FSDiscriminatorPass>(getNumFSPasses()));
#else
...
#if defined(BSPUB_COMMON)
  return getFSPassBitEnd(static_cast<sampleprof::FSDiscriminatorPass>(getNumFSPasses()));
#else
