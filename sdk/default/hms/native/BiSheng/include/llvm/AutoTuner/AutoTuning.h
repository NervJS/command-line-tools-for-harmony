...
// Options for updating compilation flow with AutoTuner.
enum AutoTuningCompileOpt {
  Inactive,         // Disabled incremental compilation.
  CoarseGrain,      // For tuning LLVMParam.
  FineGrain,        // For tuning default code regions (Loop, CallSite,
                    // Function).
  Basic,            // Same as CoarseGrain but can be applied for any code
                    // region. Can be used with ImpactRanker.
  PassManipulation, // Manipulation of passes for optimizing functions. Execute
                    // pass for a function only if specified in config file.
};
...
enum CodeRegionType {
  ...
  PassManipulator,   // Manipulation of passes for optimizing functions.
                     // Only execute pass for a function if specified in config.
  Empty,             // Empty CodeRegion.
  Invalid            // Invalid CodeRegion.
};
enum HotnessType { Unknown, Cold, Hot, };
...
public:
  ...
  inline bool operator!=(const CodeRegion &CR) const {
    return !(*this == CR);
  };
...
private:
  ...
  mutable unsigned int Invocation = 0;
...
template <> struct DenseMapInfo<CodeRegion> {
  ...
  static unsigned getHashValue(const CodeRegion &CR) {
    return static_cast<unsigned>(
        llvm::hash_combine(CR.getPassName(), CR.getType()));
  }
};
} // namespace llvm
...
public:
  ...
  /// Return true if pass-function pair exists as PassManipulator code region,
  /// and false otherwise.
  bool checkPassManipulation(std::string PassName, std::string FuncName);
  ...
  llvm::Optional<unsigned int> getCallSiteLoc(llvm::CallBase *CB);
  ...
  /// A map storing PassManipulator code region.
  std::unordered_map<std::string, std::unordered_set<std::string>>
      PassManipulatorParams;
  /// A list of passes which are specified for PassManipulator code region.
  std::unordered_set<std::string> PassManipulatorPassesList;
