...
#ifdef BSPRIV_AARCH64_PLT_INLINING
...
#ifdef BSPRIV_AARCH64_PLT_INLINING
          PLTInlining(false),
#endif // BSPRIV_AARCH64_PLT_INLINING
...
#ifdef BSPRIV_AARCH64_PLT_INLINING
    unsigned PLTInlining : 1;

  private:
    // The set of function symbols considered at LTO time to be defined and
    // callable within the current DSO.
    llvm::SmallSet<StringRef, 16> BinaryDefinedFunctions;

  public:
    std::pair<NoneType, bool> insertBinaryDefinedFunctions(StringRef Name) {
      return BinaryDefinedFunctions.insert(Name);
    }

    SmallSet<StringRef, 10>::size_type
    countBinaryDefinedFunctions(StringRef Name) const {
      return BinaryDefinedFunctions.count(Name);
    }
#endif // BSPRIV_AARCH64_PLT_INLINING
