...
#if defined(BSPUB_COMMON) && defined(BSPRIV_AARCH64_SCHEDULING)
  /// Change scheduling direction to \p SchedDirection.
  virtual void changeDirection(SchedDirection) {
    llvm_unreachable("not implemented");
  };

  /// Get current emulation cycle.
  virtual unsigned getCurrCycle() { llvm_unreachable("not implemented"); }
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_AARCH64_SCHEDULING)
  /// Bidirectional RPPressure
  IntervalPressure BiPressure;
  RegPressureTracker BiRPTracker;
#endif

public:
#if defined(BSPUB_COMMON) && defined(BSPRIV_AARCH64_SCHEDULING)
  struct SchedCost {
    SchedCost(SchedDirection D, unsigned RP, unsigned Cycles);

    static unsigned RPCostMultiplier;
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_AARCH64_SCHEDULING)
  unsigned calcRegPressureExcess(SchedDirection Direct);
  void feedDummyRPTracker(std::vector<SUnit *> BidirectionalOrder);
  std::vector<SUnit *> getCurrentSchedulingSUOrder();
  void recoverRegion(const std::vector<unsigned> &Order);
  std::vector<unsigned> getCurrentSchedulingOrder();
  std::vector<unsigned> getDefaultSchedulingOrder();
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_AARCH64_SCHEDULING)
  void schedule(SchedDirection Direct);
#endif
};

#if defined(BSPUB_COMMON) && defined(BSPRIV_AARCH64_SCHEDULING)
llvm::raw_ostream &operator<<(llvm::raw_ostream &OS,
                              const ScheduleDAGMILive::SchedCost &C);
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_AARCH64_SCHEDULING)
  void changeDirection(SchedDirection) override;

  unsigned getCurrCycle() override;
#endif
