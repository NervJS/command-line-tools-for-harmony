...
extern cl::opt<unsigned> UserLoopMicroOpBufferSize;
...
public:
  ...
  bool isExtLoadLegal(unsigned Opcode, Type *Dst, Type *Src) const {
    ...
    return getTLI()->isLoadExtLegalOrCustom(LType, ExtVT, LoadVT);
  }
#endif
...
#if defined(BSPUB_COMMON) && defined(BSPRIV_AARCH64_SCHEDULING)
    unsigned LoopMicroOpBufferSize = ST->getSchedModel().LoopMicroOpBufferSize;
    // Override sched model MicroOpBufferSize with user value.
    if (UserLoopMicroOpBufferSize.getNumOccurrences() > 0)
      LoopMicroOpBufferSize = UserLoopMicroOpBufferSize;

    if (PartialUnrollingThreshold.getNumOccurrences() > 0)
      MaxOps = PartialUnrollingThreshold;
    else if (LoopMicroOpBufferSize > 0)
      MaxOps = LoopMicroOpBufferSize;
#else
    if (PartialUnrollingThreshold.getNumOccurrences() > 0)
      MaxOps = PartialUnrollingThreshold;
  ...
  bool enableUnrollThrottling() const { return false; }
#endif
  ...
  virtual unsigned getTargetSupportedIntrinsic(const Intrinsic::ID IID) const {
    return 0;
  }
#endif
  /// @}
  ...
  getTypeBasedIntrinsicInstrCost(const IntrinsicCostAttributes &ICA,
    ...
    case Intrinsic::vector_reduce_fmul:
      return thisT()->getArithmeticReductionCost(Instruction::FMul, VecOpTy,
                                                 FMF, CostKind);
#if defined(BSPUB_COMMON) && defined(BSPRIV_COMMON)
    // vp.reudce.* may introduce additional select than vector.reduce.*
    // after expandvp pass, so we add overhead 1 for the intrinsic.
    //
    // Fix later: Select can be eliminated by enhancing instcombine or
    // by introducing lowering for vp.reduce. Therefore, the extra cost 1
