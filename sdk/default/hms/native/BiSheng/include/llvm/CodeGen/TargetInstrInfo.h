...
public:
  ...
  /// Return true if the MachineBasicBlock is a jump table lookup (for AArch64)
  virtual bool isMBBJumpTableLookup(const MachineBasicBlock &MBB) const {
    return false;
  }
  ...
  /// Returns true if MI is instruction to get the thread pointer
  virtual bool isGetThreadPointer(const MachineInstr &MI) const {
    return false;
  }
