...
public:
  ...
  enum {
    ...
    /// 21 bits should be enough for anyone OHOS_LOCAL.
  ...
  static Qualifiers removeCommonQualifiers(Qualifiers &L, Qualifiers &R) {
    ...
    // OHOS_LOCAL begin
    if (L.getMiuPtr() == R.getMiuPtr()) {
      Q.setMiuPtr(L.getMiuPtr());
      L.removeMiuPtr();
      R.removeMiuPtr();
    }
    // OHOS_LOCAL end
  ...
  // OHOS_LOCAL begin
  bool hasMiuPtr(void) const { return Mask & MiuPtrMask; }
  void setMiuPtr(MPtr type) {
    Mask = (Mask & ~MiuPtrMask) | (type << MiuPtrShift);
  }
  MPtr getMiuPtr() const { return MPtr((Mask & MiuPtrMask) >> MiuPtrShift); }
  void removeMiuPtr() { setMiuPtr(MiuPtrNone); }
  void addMiuPtr(MPtr type) {
    assert(type);
    setMiuPtr(type);
  }
  // OHOS_LOCAL end
  ...
  // OHOS_LOCAL begin
  Qualifiers withoutMiuPtr() const {
    Qualifiers qs = *this;
    qs.removeMiuPtr();
    return qs;
  }
  ...
  void addConsistentQualifiers(Qualifiers qs) {
    // OHOS_LOCAL begin
    assert(getMiuPtr() == qs.getMiuPtr() ||
           !hasMiuPtr() || !qs.hasMiuPtr());
    // OHOS_LOCAL end
  ...
  bool compatiblyIncludes(Qualifiers other) const {
    return isAddressSpaceSupersetOf(other) &&
           // OHOS_LOCAL begin
           // MiuPtr qualifiers can match, be added, or be removed and can be changed
           // only from/to MiuAddr::Any
           (getMiuPtr() == other.getMiuPtr() || !hasMiuPtr() ||
            !other.hasMiuPtr()) &&
           // OHOS_LOCAL end
...
private:
  ...
  //           |C R V|U|GCAttr|Lifetime|M   |AddressSpace|
  uint32_t Mask = 0;
...
// OHOS_LOCAL begin
  static const uint32_t MiuPtrMask = 0x600;
  static const uint32_t MiuPtrShift = 9;
// OHOS_LOCAL end
  static const uint32_t AddressSpaceMask =
      ~(CVRMask | UMask | GCAttrMask | LifetimeMask | MiuPtrMask); // OHOS_LOCAL
  static const uint32_t AddressSpaceShift = 11; // OHOS_LOCAL modify from 9 to 11
};
...
public:
  ...
  // OHOS_LOCAL begin
  /// Returns MiuPtr attribute of this type.
  inline Qualifiers::MPtr getMiuPtr() const;

  bool hasMiuPtr(void) const {
    return getQualifiers().hasMiuPtr();
  }
  // OHOS_LOCAL end
...
public:
  ...
  // OHOS_LOCAL begin
  bool hasMiuPtr(void) const { return Quals.hasMiuPtr(); }
  Qualifiers::MPtr getMiuPtr() const { return Quals.getMiuPtr(); }
  // OHOS_LOCAL end
...
// OHOS_LOCAL begin
/// Return the MPtr attribute of this type.
inline Qualifiers::MPtr QualType::getMiuPtr() const {
  return getQualifiers().getMiuPtr();
}
// OHOS_LOCAL end
