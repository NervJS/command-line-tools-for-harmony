...
/// This represents 'deadline' clause in the '#pragma omp taskgroup'
/// directive.
///
/// \code
/// #pragma omp taskgroup deadline(4)
/// \endcode
/// In this example directive '#pragma omp taskgroup' has clause 'deadline'
/// with single expression '4'.
/// If the 'deadline' clause is used then it specifies the deadline for the
/// region enclosed by the taskgroup. The parameter of the 'deadline'
/// clause must be a constant positive integer expression.
class OMPDeadlineClause : public OMPClause {
  friend class OMPClauseReader;

  /// Location of '('.
  SourceLocation LParenLoc;

  /// The deadline for the code region.
  Stmt *Deadline = nullptr;

  /// Set deadline.
  void setDeadline(Expr *DeadlineNum) { Deadline = DeadlineNum; }

public:
  /// Build 'deadline' clause.
  ///
  /// \param DeadlineNum Expression associated with this clause.
  /// \param StartLoc Starting location of the clause.
  /// \param EndLoc Ending location of the clause.
  OMPDeadlineClause(Expr *DeadlineNum, const SourceLocation StartLoc,
                    const SourceLocation LParenLoc, const SourceLocation EndLoc)
      : OMPClause(llvm::omp::OMPC_deadline, StartLoc, EndLoc),
        LParenLoc(LParenLoc), Deadline(DeadlineNum) {}

  /// Build an empty clause.
  explicit OMPDeadlineClause()
      : OMPClause(llvm::omp::OMPC_deadline, SourceLocation(),
                  SourceLocation()) {}
  ...
  /// Return deadline.
  Expr *getDeadline() const { return cast_or_null<Expr>(Deadline); }

  child_range children() { return child_range(&Deadline, &Deadline + 1); }

  const_child_range children() const {
    return const_child_range(&Deadline, &Deadline + 1);
  }
  ...
  static bool classof(const OMPClause *T) {
    return T->getClauseKind() == llvm::omp::OMPC_deadline;
  }
};
#endif
...
#ifdef BSPRIV_COMMON_OPENMP_FFRT
/// This represents 'deadline_qos' clause in the '#pragma omp taskgroup'
/// directive.
///
/// \code
/// #pragma omp taskgroup deadline_qos(user_interactive)
/// \endcode
/// In this example directive '#pragma omp taskgroup' has simple 'deadline_qos'
/// clause with kind 'user_interactive'.
class OMPDeadlineQOSClause final : public OMPClause {
  friend class OMPClauseReader;

  /// Location of '('.
  SourceLocation LParenLoc;

  /// A kind of the 'deadline_qos' clause.
  OpenMPDeadlineQOSClauseKind QOSKind = OMPC_DEADLINE_QOS_unknown;
  ...
  ///
  /// \param K Kind of clause.
  void setKind(OpenMPDeadlineQOSClauseKind K) { QOSKind = K; }
...
public:
  /// Build 'deadline_qos' clause with argument \p A ('user_interactive').
  ///
  /// \param A Argument of the clause ('deadline_qos').
  /// \param ALoc Starting location of the argument.
  /// \param StartLoc Starting location of the clause.
  /// \param LParenLoc Location of '('.
  /// \param EndLoc Ending location of the clause.
  OMPDeadlineQOSClause(OpenMPDeadlineQOSClauseKind A, const SourceLocation ALoc,
                       const SourceLocation StartLoc,
                       const SourceLocation LParenLoc,
                       const SourceLocation EndLoc)
      : OMPClause(llvm::omp::OMPC_deadline_qos, StartLoc, EndLoc),
        LParenLoc(LParenLoc), QOSKind(A), KindKwLoc(ALoc) {}

  /// Build an empty clause.
  OMPDeadlineQOSClause()
      : OMPClause(llvm::omp::OMPC_deadline_qos, SourceLocation(),
                  SourceLocation()) {}
  ...
  /// Returns kind of the clause.
  OpenMPDeadlineQOSClauseKind getKind() const { return QOSKind; }
  ...
  static bool classof(const OMPClause *T) {
    return T->getClauseKind() == llvm::omp::OMPC_deadline_qos;
  }
};
#endif
